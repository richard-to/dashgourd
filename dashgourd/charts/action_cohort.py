...
def create_action_cohort(db, collection, options):
    """Creates data action based cohorts
    
    This is more of a short term solution to handle 
    cohorts based on action dates.
    ...
    of putting actions as top level actions.
    
    Restrictions:
    
    Action groups can only be grouped by one value for now.
    
    Action that are grouped must be connected by a meta value 
